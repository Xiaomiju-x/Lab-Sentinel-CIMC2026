                                  ...
                                  float *workspace, uint32_t workspace_elems)
{
    ...
    if (maximum > workspace_elems / 2U ||
        maximum > SIZE_MAX / (2U * sizeof(float))) return F2RT_ERR_WORKSPACE;
    b = workspace + maximum;
    for (layer = 0U; layer < layers; ++layer) {
        uint32_t tensor_index = tensor_start + layer * 2U;
    ...
    if (current_count > output_capacity || current_count > SIZE_MAX / sizeof(float)) return F2RT_ERR_OUTPUT;
                        ...
                        f2rt_model_t *model)
{
    ...
    if (candidate.tensor_count == 0U ||
        ...
        candidate.input_elems > SIZE_MAX / sizeof(float) ||
        candidate.output_elems > SIZE_MAX / sizeof(float) ||
        candidate.workspace_elems > SIZE_MAX / sizeof(float)) {
        return F2RT_ERR_BOUNDS;
    }
                                     ...
                                     float *workspace, uint32_t workspace_elems)
{
    ...
    if (hidden_count == 0U || hidden_count > workspace_elems ||
        hidden_count > SIZE_MAX / sizeof(float) || alpha_w.ndim != 2U || alpha_w.dims[0] != 1U ||
                                ...
                                float *workspace, uint32_t workspace_elems)
{
    ...
    if (output_elems != model->output_elems || output_elems > output_capacity ||
        output_elems > SIZE_MAX / sizeof(float)) return F2RT_ERR_OUTPUT;
                             ...
                             float *workspace, uint32_t workspace_elems)
{
    ...
    if (model == NULL || input == NULL || output == NULL || workspace == NULL ||
        ...
        model->input_elems > SIZE_MAX / sizeof(float) ||
        model->output_elems > SIZE_MAX / sizeof(float) ||
        model->workspace_elems > SIZE_MAX / sizeof(float)) {
        return F2RT_ERR_ARGUMENT;
    }
                                    ...
                                    float *workspace, uint16_t *argmax_out)
{
    ...
    if (kv_each64 > UINT32_MAX || required64 > model->workspace_elems ||
        required64 > SIZE_MAX / sizeof(float)) {
                                ...
                                float *workspace, uint32_t workspace_elems)
{
    ...
    if (model == NULL || prompt == NULL || generated == NULL || workspace == NULL ||
        ...
        model->workspace_elems > SIZE_MAX / sizeof(float) ||
        generated_capacity > SIZE_MAX / sizeof(uint16_t)) return F2RT_ERR_ARGUMENT;
