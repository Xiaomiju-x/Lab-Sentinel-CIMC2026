#!/usr/bin/env python3
"""Stage IPOP rows for the P057 NIR-emitter excitation-peak contract."""
...
def main() -> int:
    ...
    with source.open("r", encoding="utf-8-sig", newline="") as handle:
        for row in csv.DictReader(handle):
            ...
            excitation = number(row["1st Excitation max. (nm)"])
            if not math.isfinite(emission) or emission < 700.0 or not math.isfinite(excitation) or excitation <= 0.0:
                continue
            ...
            monitoring = number(row["Monitoring energy (nm)"], 0.0)
            excitation_2 = number(row["2nd Excitation max. (nm)"], 0.0)
            excitation_3 = number(row["3rd Excitation max. (nm)"], 0.0)
                ...
                (
                    ...
                    np.asarray(
                        [
                            ...
                            temperature / 2000.0, emission / 2000.0, monitoring / 2000.0,
                            excitation_2 / 2000.0, float(excitation_2 > 0.0),
                            excitation_3 / 2000.0, float(excitation_3 > 0.0),
                            0.0,
                        ],
                        dtype=np.float32,
                    ),
            ...
            rows.append(
                {
                    "x": feature, "y": excitation, "family": family,
    ...
    # The preregistered train-only host-family median is also the residual prior.
    ...
    if min(counts.values()) < 14 or overlap:
        raise RuntimeError(f"P057_SPLIT_GATE:{counts}:overlap={overlap}")
    output = root / "data" / "staged_ipop_p057_exact_v1" / f"{candidate_id}.npz"
    ...
    metadata = {
        "schema": "cimc.forge200.ipop-p057-exact-staged.v1", "created_at_utc": datetime.now(timezone.utc).isoformat(),
        ...
        "record_filter": "emission_max_nm_ge_700_and_positive_first_excitation_peak_nm",
        "target_transform": "none_wavelength_nm",
        "target_scope": "excitation peak of a literature-curated NIR-emitting phosphor; not a claim that the excitation wavelength itself is NIR",
        "feature_contract": "host_composition_118+dopant_presence_118+concentrations+valencies+temperature+NIR_emission_and_monitoring_context+secondary_excitation_peaks_with_availability+explicit_site_missingness_mask+train_only_host_median_residual_prior",
        "input_contract": contract["input_contract"],
        "input_contract_state": "SATISFIED_FOR_NIR_EMITTER_SCOPE_WITH_SECONDARY_PEAK_SPECTRUM_SUMMARY_AND_EXPLICIT_SITE_MISSINGNESS_MASK_NO_SITE_VALUE_FABRICATED",
        "target_label": contract["target_label"], "baseline": contract["baseline"],
        "baseline_fit": "TRAIN_ONLY_HOST_FAMILY_MEDIAN_WITH_GLOBAL_FALLBACK",
    ...
    receipt = {"schema": "cimc.forge200.ipop-p057-exact-staging.v1", "created_at_utc": datetime.now(timezone.utc).isoformat(), "status": "PASS", **content, "content_root_sha256": hashlib.sha256(canonical_bytes(content)).hexdigest()}
    write_json(root / "evidence" / "ipop_p057_exact_staging.v1.json", receipt)
