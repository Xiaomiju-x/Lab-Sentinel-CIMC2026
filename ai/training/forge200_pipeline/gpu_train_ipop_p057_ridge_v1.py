#!/usr/bin/env python3
"""Train the P057 train-fitted host-prior residual ridge model on CUDA."""
...
def metrics(y: np.ndarray, prediction: np.ndarray) -> dict[str, float]:
    ...
    return {
        "MAE_nm": mae,
        "p95_error_nm": p95,
        "RMSE_nm_not_primary": float(np.sqrt(np.mean(error**2))),
        "composite_higher_is_better": 0.5 / (1.0 + mae / 100.0) + 0.5 / (1.0 + p95 / 200.0),
    }
...
def main() -> int:
    ...
    dataset_path = root / "data" / "staged_ipop_p057_exact_v1" / f"{candidate_id}.npz"
    ...
    for value in lambdas:
        ...
        prediction = prior[indices["validation"]] + z[indices["validation"]] @ weight_np
    ...
    torch.onnx.export(model, torch.from_numpy(model_input[indices["test"][:1]]).to(device), onnx_path, input_names=["preprocessed_host_dopant_spectrum_summary_and_host_prior"], output_names=["excitation_peak_nm"], dynamic_axes={"preprocessed_host_dopant_spectrum_summary_and_host_prior": {0: "batch"}, "excitation_peak_nm": {0: "batch"}}, opset_version=17, dynamo=False)
    import onnx
    ...
    output_schema = {"task_kind": "regression", "shape": [None, 1], "units": "nm", "semantics": "excitation_peak_of_NIR_emitter", "authority": 0}
    ...
    gates = {
        ...
        "G3_three_seed_mean_beats_preregistered_baseline": aggregate["mean"] > baseline["composite_higher_is_better"] and result["MAE_nm"] < baseline["MAE_nm"] and result["p95_error_nm"] < baseline["p95_error_nm"],
        "G4_mean_variance_worst_reported": len(seed_reports) == 3,
        "G5_parameter_and_weight_caps": parameter_count <= 36000 and package["bytes"] - 256 <= 48 * 1024,
        "G6_quantized_model_beats_baseline": quant_metrics["composite_higher_is_better"] > baseline["composite_higher_is_better"] and quant_metrics["MAE_nm"] < baseline["MAE_nm"] and quant_metrics["p95_error_nm"] < baseline["p95_error_nm"],
    ...
    exact = {
        "schema": "cimc.forge200.ipop-p057-exact-audit.v1", "created_at_utc": datetime.now(timezone.utc).isoformat(),
    ...
    write_json(output / "calibration_ood.json", {"schema": "cimc.forge200.calibration-ood.v1", "test_p95_error_nm": result["p95_error_nm"], "quant_max_abs_error_nm": parity, "authority": 0})
    ...
    (output / "model_card.md").write_text(f"# {candidate_id} model card\n\n- Status: `{status}`\n- Model: train-only host median plus CUDA ridge residual, lambda `{selected_lambda}`.\n- Scope: excitation peak of literature-curated NIR-emitting phosphors; the excitation wavelength itself is not claimed to be NIR.\n- Three fixed seeds: deterministic mean `{aggregate['mean']:.6f}`, variance `{aggregate['variance']:.8f}`, worst `{aggregate['worst']:.6f}`.\n- Test MAE: `{result['MAE_nm']:.6f}` vs preregistered baseline `{baseline['MAE_nm']:.6f}`.\n- Authority: `0`; unified GD32 board acceptance remains pending.\n", encoding="utf-8")
    promotion = {
    ...
    closure = {"schema": "cimc.forge200.ipop-p057-exact-closure.v1", "created_at_utc": datetime.now(timezone.utc).isoformat(), "status": "PASS" if passed else "PARTIAL", **closure_content, "content_root_sha256": hashlib.sha256(canonical_bytes(closure_content)).hexdigest()}
    write_json(root / "evidence" / "ipop_p057_exact_closure.v1.json", closure)
    print(json.dumps({"status": closure["status"], "candidate_id": candidate_id, "model_mae_nm": result["MAE_nm"], "baseline_mae_nm": baseline["MAE_nm"], "runtime_seconds": promotion["runtime_seconds"]}, sort_keys=True))
