#!/usr/bin/env python3
"""Materialize content-verified ModelBank v6 from host closure v6."""
...
def main() -> int:
    ...
    closure_path = root / "evidence" / "host_closure.v6.json"
    ...
    manifest = {
        "schema": "cimc.forge200.modelbank-build.v6",
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "status": "HOST_MODELBANK_BUILT_ASSET_FLOOR_MET_IF_BOARD_PASSES",
        "root": str(output),
    ...
    manifest_path = output / "MANIFEST.v6.json"
    ...
    write(root / "evidence" / "modelbank_build.v6.json", receipt)
    print(
        json.dumps(
            {
