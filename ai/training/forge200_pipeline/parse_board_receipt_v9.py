#!/usr/bin/env python3
"""Parse the final unified ModelBank/RAG/VeriProcess GD32 UART receipt."""
...
def main() -> int:
    ...
    veriprocess = one(events, "VERIPROCESS")
    if (
        veriprocess is None
        or as_int(veriprocess, "ledger_generation") < 1
        or as_int(veriprocess, "ledger_records") < 2
        or as_int(veriprocess, "chrono_events") != 11
        or as_int(veriprocess, "independent_families") < 2
        or as_int(veriprocess, "ds3231") != 1
        or as_int(veriprocess, "wal_recovered") not in {0, 1}
        or as_int(veriprocess, "sintergraph_frozen") != 1
        or as_int(veriprocess, "authority") != 0
    ):
        errors.append("VERIPROCESS_GATE")
    ...
    rag = one(events, "RAG120")
    if (
        rag is None
        or as_int(rag, "queries") != 120
        or as_int(rag, "safe") != 120
        or as_int(rag, "source_bound") <= 0
        or as_int(rag, "negative_refused") != 60
        or as_int(rag, "warm_hits") < 96
        or as_int(rag, "warm_hit_percent") < 80
        or as_int(rag, "cold_p95_ms") > 20000
        or as_int(rag, "cold_p99_ms") > 30000
        or as_int(rag, "warm_p95_ms") > 8000
        or as_int(rag, "warm_p99_ms") > 12000
    ):
        errors.append("RAG120_GATE")
    rag_stage = one(events, "RAG_STAGE")
    required_rag_stages = (
        "load_support_p95_ms", "route_retrieve_p95_ms", "load_lm_p95_ms",
        "generate_p95_ms", "unload_p95_ms", "nli_p95_ms",
        "commit_p95_ms", "zeroize_p95_ms",
    )
    if rag_stage is None or any(as_int(rag_stage, name) < 0 for name in required_rag_stages):
        errors.append("RAG_STAGE_GATE")

    swap = one(events, "SWAP1000")
    if (
        swap is None
        or as_int(swap, "swap_loads") != 1000
        or as_int(swap, "same_model_reload") != 10
        or as_int(swap, "mid_cancel") != 4
    ...
    if (
        ...
        or as_int(final, "rag120_safe") != 120
        or as_int(final, "rag120_source_bound") <= 0
        or as_int(final, "soak_rag_queries") != 288
    ...
    result = {
        "schema": "cimc.forge200.gd32-board-acceptance.v9",
        ...
        "forge200": {
            ...
            "same_model_reload": as_int(swap or {}, "same_model_reload"),
            "mid_load_cancel": as_int(swap or {}, "mid_cancel"),
            "rag_queries": as_int(rag or {}, "queries"),
            "rag_safe": as_int(rag or {}, "safe"),
            "rag_source_bound": as_int(rag or {}, "source_bound"),
            "veriprocess_ledger_records": as_int(veriprocess or {}, "ledger_records"),
