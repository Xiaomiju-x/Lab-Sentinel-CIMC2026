"""
export_flagship_to_c.py — quantize the WINNING flagship LM to INT8 and emit the
same 3 firmware headers the 0.6M nano-LM used (nanolm_weights.h / nanolm_vocab.h
/ nanolm_golden.h), so the flagship DROPS IN as the always-on generative LM with
only a recompile (ai_nanolm.c is fully config-driven: D/DH/NH/NL/FF/MS/V are all
NLM_* macros, buffers are short gen[NLM_MAXSEQ]).

Difference vs export_nanolm_to_c.py: the config + vocab live IN the checkpoint
(train_flagship saves {"model","vocab","cfg","tok2id","id2tok","control"}), not
in module constants. Pass --tag to pick which swept size to ship.

Same INT8 scheme as the project's nn_linear_int8: per-output-row symmetric INT8
weights, fp32 bias/scale/LayerNorm/pos, fp32 compute (M7 has no INT8 SIMD -> the
win is Flash/D-cache, honestly labelled). Golden = dequantize back to fp32, load
into the EXACT deployed model, greedy-generate demos + dump last-pos logits; the
C engine must reproduce both byte-for-byte.

Run (after the sweep, on the box with the .pt):
  python export_flagship_to_c.py --tag m1p35
"""
import argparse
import json
...
from train_flagship import GPT, parse_tag
...
def main():
    ...
    ap.add_argument(
        ...
        required=True,
        type=parse_tag,
        help="which swept size to ship, e.g. m1p35",
    )
    args = ap.parse_args()

    ck = torch.load(
        HERE / f"flagship_{args.tag}.pt",
        map_location="cpu",
        weights_only=True,
    )
    cfg = ck["cfg"]
    ...
    tok2id = ck["tok2id"]
    control = ck["control"]
    D_MODEL, N_LAYERS, N_HEADS = cfg["d_model"], cfg["n_layers"], cfg["n_heads"]
    D_FF, MAX_SEQ = cfg["d_ff"], cfg["max_seq"]
    ...
    tok_q, tok_s = quant_rows(model.tok.weight)
    ...
    # demo states (same 12-slot schema; cover normal + 2 faults) for golden + selftest
    demos = [
    ...
    print(f"=== flagship [{args.tag}] deployed (INT8-dequant) greedy demos ===")
    ...
    w = (f"/* nanolm_weights.h - AUTO-GENERATED by export_flagship_to_c.py (tag={args.tag}). Do not edit.\n"
         " * GD32 edge flagship LM: INT8 weight-only (per-row symmetric) + fp32 scales/bias/LN/pos.\n"
         " * Distilled from DeepSeek V4. Weights stored in Flash (.rodata). */\n"
    ...
    v = (f"/* nanolm_vocab.h - AUTO-GENERATED (flagship tag={args.tag}). id->UTF8 detok + control ids + dims. */\n"
    ...
    g = (f"/* nanolm_golden.h - AUTO-GENERATED (flagship tag={args.tag}). Demo ctx + greedy ids + ref logits. */\n"
    ...
    print(f"\nflagship [{args.tag}]: vocab {V} dims d{D_MODEL}/L{N_LAYERS}/ff{D_FF}/seq{MAX_SEQ}")
    print(f"INT8 weight bytes ~{int8_bytes/1024:.0f}KB + pos fp32 {pos.size*4/1024:.0f}KB; headers {sz/1024:.0f}KB")
