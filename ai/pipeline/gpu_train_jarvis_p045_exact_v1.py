#!/usr/bin/env python3
"""Train and package the exact JARVIS competing-phase ranker P045."""
...
def ranking_metrics(y: np.ndarray, score: np.ndarray, groups: np.ndarray) -> dict[str, float | int]:
    ...
    for group in np.unique(groups):
        ...
        best_truth_local_index = int(true_order[0])
        predicted_position = int(np.flatnonzero(pred_order == best_truth_local_index)[0]) + 1
        reciprocal_ranks.append(1.0 / predicted_position)
        top1_values.append(float(predicted_position == 1))
    result = {
    ...
    result["primary_composite"] = float(np.mean([result["NDCG_at_5"], result["MRR"], result["top1_accuracy"]]))
    return result
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    candidate_id = "CAND-P-045"
    dataset = root / "data" / "staged_jarvis_p045_exact_v1" / f"{candidate_id}.npz"
    ...
    baseline_feature_index = int(metadata["baseline_feature_index"])
    baseline_weight = np.zeros(x.shape[1], dtype=np.float32)
    baseline_weight[baseline_feature_index] = std[baseline_feature_index]
    baseline_bias = float(mean[baseline_feature_index])

    class RankMLP(nn.Module):
        def __init__(self) -> None:
            super().__init__()
            self.net = nn.Sequential(
                nn.Linear(x.shape[1], 160),
                nn.GELU(),
                nn.Linear(160, 64),
                nn.GELU(),
                nn.Linear(64, 1),
    ...
    for seed in SEEDS:
        ...
        for epoch in range(args.max_epochs):
            ...
            if score > best_score + 1e-5:
                ...
                patience += 1
            heartbeat(heartbeat_path, candidate_id, "TRAIN_JARVIS_P045_EXACT", seed, epoch)
    ...
    torch.onnx.export(model, torch.from_numpy(x[test[:1]]).to(device), onnx_path, input_names=["composition_candidate_phase_and_availability_features"], output_names=["competing_phase_rank_score"], dynamic_axes={"composition_candidate_phase_and_availability_features": {0: "batch"}, "competing_phase_rank_score": {0: "batch"}}, opset_version=17, dynamo=False)
    import onnx

    onnx.checker.check_model(onnx.load(onnx_path))
    output_schema = {"shape": [None, 1], "semantics": "within_chemical_system_competing_phase_rank_score", "ranking": "ascending", "authority": 0}
    ...
    evaluation = {
        "schema": "cimc.forge200.jarvis-p045-contract-exact-evaluation.v1",
...
def main() -> int:
    ...
    try:
        ...
        output = args.artifact_root.resolve() / "CAND-P-045"
        write_json(output / "failure.json", {"schema": "cimc.forge200.job-failure.v3", "status": "FAIL_CLOSED", "candidate_id": "CAND-P-045", "authority": 0, "error_type": type(exc).__name__, "error": str(exc), "utc": datetime.now(timezone.utc).isoformat()})
        raise
