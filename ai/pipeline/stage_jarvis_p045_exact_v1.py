#!/usr/bin/env python3
"""Stage source-bound JARVIS competing-phase rankings for P045."""
...
def main() -> int:
    ...
    if len(filtered) != 64066 or len(by_system) != 9709:
        raise RuntimeError(f"COMPETING_PHASE_FILTER_GATE:records={len(filtered)}:groups={len(by_system)}")
        ...
        for row in rows:
            ...
            material = np.asarray(
                [
                    ...
                    numeric_or_zero(row.get("optb88vdw_bandgap")),
                    numeric_or_zero(row.get("density")),
                    numeric_or_zero(row.get("spg_number")) / 230.0,
                    float(len(set(row["atoms"]["elements"]))) / 4.0,
    ...
    with contract_path.open("r", encoding="utf-8-sig", newline="") as handle:
        contract = next(row for row in csv.DictReader(handle, delimiter="\t") if row["candidate_id"] == "CAND-P-045")
    output = root / "data" / "staged_jarvis_p045_exact_v1" / "CAND-P-045.npz"
    ...
    np.savez_compressed(
        ...
        candidate_id=np.asarray("CAND-P-045"),
    )
    metadata = {
        "schema": "cimc.forge200.jarvis-p045-staged.v1",
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "status": "PASS",
        "candidate_id": "CAND-P-045",
        "truth_class": "OPEN_COMPUTED_DFT_COMPETING_PHASE_RANKING",
        ...
        "candidate_filter": "2_TO_4_ELEMENTS_AND_AT_LEAST_3_PUBLISHED_STRUCTURES_PER_CHEMICAL_SYSTEM",
        "scope_boundary": "computed competing-phase thermodynamic ranking only; precursor and synthesis-temperature fields are explicitly unavailable and are not fabricated",
        "input_contract": contract["input_contract"],
        "input_contract_state": "SATISFIED_TARGET_COMPOSITION_AND_CANDIDATE_PHASES_WITH_EXPLICIT_PRECURSOR_AND_TEMPERATURE_AVAILABILITY_MASKS_ZERO",
        "input_availability_masks": {"precursors": 0, "temperature": 0},
        "target_label": contract["target_label"],
        "target_field": "ehull",
        "target_semantics": "published JARVIS energy above convex hull; lower is more stable",
        "baseline": contract["baseline"],
        "baseline_execution": "published formation_energy_peratom ascending within each chemical system",
        "baseline_feature_index": int(x_array.shape[1] - 8),
    ...
    receipt = {
        "schema": "cimc.forge200.jarvis-p045-staging.v1",
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "status": "PASS",
        "record": metadata,
    ...
    write_json(root / "evidence" / "jarvis_p045_staging.v1.json", receipt)
