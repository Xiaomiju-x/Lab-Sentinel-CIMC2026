#!/usr/bin/env python3
"""Train the P058 train-fitted family-prior residual ridge model on CUDA."""
...
def metrics(y: np.ndarray, prediction: np.ndarray, half_width: float) -> dict[str, float]:
    ...
    return {
        "mae_log_microseconds": mae,
        "rmse_log_microseconds": float(np.sqrt(np.mean(error**2))),
...
def main() -> int:
    ...
    dataset_path = root / "data" / "staged_ipop_exact_v1" / f"{candidate_id}.npz"
    ...
    baseline_half_width = float(np.quantile(np.abs(baseline_prediction[indices["validation"]] - y[indices["validation"]]), 0.90))
    baseline = metrics(y[indices["test"]], baseline_prediction[indices["test"]], baseline_half_width)
    ...
    torch.onnx.export(model, torch.from_numpy(model_input[indices["test"][:1]]).to(device), onnx_path, input_names=["preprocessed_features_and_family_prior"], output_names=["log_lifetime"], dynamic_axes={"preprocessed_features_and_family_prior": {0: "batch"}, "log_lifetime": {0: "batch"}}, opset_version=17)
    import onnx
    ...
    output_schema = {"task_kind": "regression", "shape": [None, 1], "units": "log10_microseconds", "authority": 0}
    ...
    gates = {
        ...
        "G3_three_seed_mean_beats_preregistered_baseline": aggregate["mean"] > baseline["composite_higher_is_better"] and result["mae_log_microseconds"] < baseline["mae_log_microseconds"],
        "G4_mean_variance_worst_reported": len(seed_reports) == 3,
        "G5_parameter_and_weight_caps": parameter_count <= 34000 and package["bytes"] - 256 <= 44 * 1024,
        "G6_quantized_model_beats_baseline": quant_metrics["composite_higher_is_better"] > baseline["composite_higher_is_better"] and quant_metrics["mae_log_microseconds"] < baseline["mae_log_microseconds"],
    ...
    exact = {
        "schema": "cimc.forge200.ipop-p058-exact-audit.v1", "created_at_utc": datetime.now(timezone.utc).isoformat(),
    ...
    write_json(output / "calibration_ood.json", {"schema": "cimc.forge200.calibration-ood.v1", "interval_half_width": half_width, "quant_max_abs_error": parity, "authority": 0})
    ...
    closure = {"schema": "cimc.forge200.ipop-exact-closure.v1", "created_at_utc": datetime.now(timezone.utc).isoformat(), "status": "PASS" if passed else "PARTIAL", **closure_content, "content_root_sha256": hashlib.sha256(canonical_bytes(closure_content)).hexdigest()}
    write_json(root / "evidence" / "ipop_exact_closure.v1.json", closure)
    print(json.dumps({"status": closure["status"], "candidate_id": candidate_id, "model_mae": result["mae_log_microseconds"], "baseline_mae": baseline["mae_log_microseconds"], "runtime_seconds": promotion["runtime_seconds"]}, sort_keys=True))
